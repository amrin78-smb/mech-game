...
import { ATLAS, bgGroundKey, bgRuinsKey, bgSkyKey } from '../AssetKeys';
...
import portraitVexUrl from '../../assets/sprites/portrait_vex.png';
import portraitMaraUrl from '../../assets/sprites/portrait_mara.png';
import titleEmblemUrl from '../../assets/sprites/title_emblem.png';
...
const ZONE_ONE_SPRITES: ReadonlyArray<readonly [string, string]> = [
  ...
  ['portrait_vex', portraitVexUrl],
  ['portrait_mara', portraitMaraUrl],
  ['title_emblem', titleEmblemUrl],
];

/**
 * Parallax that has been through the AI art pass. A theme missing from here
 * falls back to the generated strips in public/art, so authoring a zone with a
 * new theme needs a palette in tools/art and `npm run art`, not a code change.
 */
...
export class PreloadScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    // Every theme any level asks for, derived from the level data so a new
    // zone does not have to be registered anywhere.
    const themes = [...new Set(levels.map((level) => level.background))];

    if (USE_PLACEHOLDER_ART) {
      for (const theme of themes) {
        this.loadGeneratedStrips(theme);
      }
      return;
    }
    ...
    const withAiArt = new Set(AI_BACKGROUNDS.map(([key]) => key));
    for (const [key, url] of AI_BACKGROUNDS) {
      this.load.image(key, url);
    }
    for (const theme of themes) {
      // A theme the art pass has not reached yet keeps its generated strips.
      if (withAiArt.has(bgSkyKey(theme))) continue;
      this.loadGeneratedStrips(theme);
    }
  }

  private loadGeneratedStrips(theme: string): void {
