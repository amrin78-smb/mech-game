...
Shooting feels heavy. Every shot has recoil, flash, noise and consequence. Juice is a feature, not polish.
The guns fight, the player commands. Every weapon auto fires, so a level can be played nearly hands off, but the player who intervenes wins harder: manual precision shots land bonus damage, and picking the focus target decides which threats die first.
Every level changes the question. New enemy compositions force different weapon and upgrade answers. No level should be beaten by the same loadout mindlessly.
2. Core loop
